...
function main() {
    ...
    try {
        ...
        for (const line of data) {
            const values: number[] = parseIntegers(line);
    ...
    for (const history of histories) {
        ...
        subLists[subLists.length - 1].unshift(0);

        for (let i = subLists.length - 2; i >= 0; i--) {
            const extrapolatedValue = subLists[i][0] - subLists[i + 1][0];
            subLists[i].unshift(extrapolatedValue);
        }

        extrapolatedValues.push(subLists[0][0]);
    }
...
function parseIntegers(input: string): number[] {
    const nums: number[] = [];
