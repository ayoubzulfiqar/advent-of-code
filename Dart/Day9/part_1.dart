...
void main() {
  ...
  try {
    ...
    for (String line in data) {
      List<int> values = parseInts(line);
  ...
  for (List<int> history in histories) {
    ...
    for (int i = subLists.length - 2; i >= 0; i--) {
      int extrapolatedValue = subLists[i].last + subLists[i + 1].last;
      subLists[i].add(extrapolatedValue);
    }

    extrapolatedValues.add(subLists.first.last);
  }
